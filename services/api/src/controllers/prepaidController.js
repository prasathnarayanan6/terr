...
export async function getHouseForPayment(req, res) {
  ...
  try {
    const data = getHouseInfo(zone_id, houseId);
...
export async function getPrepaidOverview(req, res) {
  ...
  try {
    const data = getZoneOverview(zone_id);
...
export async function rechargeHouseCredit(req, res) {
  ...
  try {
    const house = rechargeHouse(zone_id, house_id, parsed);
    return res.json({
      success: true,
      house,
...
export async function updateValveStatus(req, res) {
  ...
  try {
    const house = setValve(zone_id, house_id, action);
    return res.json({
      success: true,
      house,
